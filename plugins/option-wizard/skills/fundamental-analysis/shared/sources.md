...
| **SEC filing (10-K / 10-Q / 13F / 13D / DEF 14A)** | Audited annuals, beneficial ownership, proxy material | `[10-K FY<year>, p. <page>]` with markdown link to SEC EDGAR URL if available |
...
| **Tier-1 news (WSJ / FT / Bloomberg / Reuters / CNBC)** | Activist disclosures, CEO changes, M&A, restructuring | `[<Outlet>, <YYYY-MM-DD>]` with link if pulled via WebSearch |
...
- **Invent SEC EDGAR URLs.** Either WebFetch the actual filing page or omit the link. EDGAR URLs follow a stable pattern but the accession numbers must be real.
...
- **Use Yahoo Finance as primary fundamental source.** Cross-validation only — primary is UW or the actual 10-K. CLAUDE.md global rule (option-wizard CLAUDE.md inherits): "Never Yahoo Finance as primary."
...
### Filings
- [^10k]    <TICKER> 10-K FY<year> — [SEC EDGAR](https://...) (link only if WebFetched)
- [^13f-Q1] <Holder> Q1 2026 13F — [SEC EDGAR](https://...)

### News
- [^wsj-1]  WSJ "Headline", YYYY-MM-DD — [link](https://...)
- [^cnbc-1] CNBC "Headline", YYYY-MM-DD — [link](https://...)
